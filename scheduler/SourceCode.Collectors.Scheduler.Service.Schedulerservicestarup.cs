...
namespace Microsoft.CloudMine.SourceCode.Collectors.Scheduler.Service
{
    public class SchedulerServiceStartup : ServiceStartupBase
    {
        public override void ConfigureServices(IServiceCollection serviceCollection, IConfiguration configuration)
        {
            ...
            string gitCacheType = configuration.GetRequiredSection(SettingConstants.GitCacheType).Get<string>() ;
            switch (gitCacheType)
            {
                case "Redis":
                    serviceCollection.Addsingleton<GitCacheFactory,RedisGitCacheFactory>();
                    break;
                default:
                    throw new Exception($"Unknown {SettingConstants.GitCacheType}: {gitCacheType}");
